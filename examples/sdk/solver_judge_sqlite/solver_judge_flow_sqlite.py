...
from rllm.sdk import ContextVarSession, SqliteSessionStorage, StepView, get_chat_client_async
...
class SolverJudgeWorkflow(Workflow):
    def __init__(self, rollout_engine: RolloutEngine, n_solutions: int = 2, reward_function: RewardFunction = None, db_path: str | None = None, **kwargs):
        super().__init__(rollout_engine, **kwargs)
        self.n_solutions = n_solutions
        self.reward_function = reward_function
        # Create SQLite storage that will be shared across all sessions
        self.storage = SqliteSessionStorage(db_path=db_path)
        self.solver = Solver(storage=self.storage)
        self.judge = Judge(storage=self.storage)
