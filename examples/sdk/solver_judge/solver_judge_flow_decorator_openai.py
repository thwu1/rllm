"""OpenAI-based version of solver-judge workflow for fast testing with real proxy.

This uses OpenAI models via LiteLLM proxy to test the full proxy integration
including trace collection, routing, and flush mechanisms.

Cost: ~$0.05-0.20 for a quick test run
Time: 3-7 minutes
"""
...
class Solver:
    def __init__(self, **kwargs):
        # Use real LiteLLM proxy with OpenAI model
        self.client = get_chat_client_async(base_url="http://localhost:4000/v1", api_key="EMPTY", model="gpt-3.5-turbo")

    @trajectory(name="solver")
    async def generate_solution(self, problem: str):
        """Generate a solution using @step decorator with OpenAI via proxy."""
...
class Judge:
    ...
    async def judge_solutions(self, problem: str, solutions: list[str]):
        """Judge solutions using @step decorator with OpenAI via proxy."""
...
class SolverJudgeWorkflowDecoratedOpenAI(Workflow):
    """OpenAI version using @trajectory and real LiteLLM proxy."""
