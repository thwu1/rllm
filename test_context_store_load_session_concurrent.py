...
# Use the session-concurrent tracer with tuned settings
tracer = LLMTracer(
    context_store=store,
    project="rllm-agent-omni-engine",
    max_queue_size=10_000,
    max_active_sessions=32,  # Controls both session and store concurrency
)
...
async def main(max_concurrent: int = 512, num_repeat: int = 2):
    ...
    print("Tracer: session-concurrent implementation")
