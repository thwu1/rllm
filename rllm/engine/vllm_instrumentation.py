...
This module provides monkey patching to add prompt_token_ids and response_token_ids
to vLLM's OpenAI-compatible API responses for versions before 0.10.2.
...
Example:
    ...
    >>> instrument_vllm()
    >>> # Now all vLLM responses include token IDs
"""
...
def instrument_vllm(force: bool = False) -> bool:
    ...
    This instrumentation adds `prompt_token_ids` and `response_token_ids` fields
    to vLLM's ChatCompletionResponse for versions < 0.10.2.
    ...
    try:
        ...
        class ChatCompletionResponsePatched(ChatCompletionResponse):
            """Extended ChatCompletionResponse with token IDs."""
        ...
        async def chat_completion_full_generator(
            ...
            """Patched generator that extracts token IDs."""
            ...
            async def _generate_interceptor():
                ...
                async for res in result_generator:
                    ...
                    print(f"🔍 [VLLM_INSTRUMENT] Extracted token IDs: prompt={len(prompt_token_ids) if prompt_token_ids else 0}, response={len(response_token_ids) if response_token_ids else 0}")
                    logger.debug(f"🔍 Extracted token IDs: prompt={len(prompt_token_ids) if prompt_token_ids else 0}, response={len(response_token_ids) if response_token_ids else 0}")
            ...
            print(f"📝 [VLLM_INSTRUMENT] Adding token IDs to response: prompt={len(prompt_token_ids) if prompt_token_ids else 0}, response={len(response_token_ids) if response_token_ids else 0}")
            logger.debug(f"📝 Adding token IDs to response: prompt={len(prompt_token_ids) if prompt_token_ids else 0}, response={len(response_token_ids) if response_token_ids else 0}")
            ...
            print(f"✅ [VLLM_INSTRUMENT] Response has token IDs: prompt_token_ids={hasattr(response, 'prompt_token_ids')}, response_token_ids={hasattr(response, 'response_token_ids')}")
            logger.debug(f"✅ Response has token IDs: prompt_token_ids={hasattr(response, 'prompt_token_ids')}, response_token_ids={hasattr(response, 'response_token_ids')}")
