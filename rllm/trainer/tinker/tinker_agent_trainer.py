...
class TinkerAgentTrainer:
    ...
    def __init__(
        ...
        assert sampling_params.get(
            "temperature", 1.0
        ) == 1.0 and sampling_params.get(
            "top_p", 1.0
        ) == 1.0, "temperature and top_p must be 1.0 for tinker agent trainer"
    ...
    async def _fit_agent_async(self):
        ...
        start_batch, sampling_client = await self.trainer.initialize_async(
            resume_from_checkpoint=True)

        # Validation before training
        if self.config.trainer.get("val_before_train",
                                   False) and self.val_dataloader:
            self.agent_execution_engine.rollout_engine.set_sampling_client(
                sampling_client)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_data in self.train_dataloader:
                ...
                batch_data = self.build_interleave_batch(
                    batch_data, self.config.training.group_size)
                ...
                self.agent_execution_engine.rollout_engine.set_sampling_client(
                    sampling_client)
                time_metrics["time/load_weights"] = time.time(
                ) - t_load_weights_start
                ...
                num_minibatches = self.config.training.get(
                    "num_minibatches", 1)
                ...
                async for minibatch_episodes in self.generate_agent_episodes(
                    ...
                    if minibatch_count == 1:
                        time_metrics[
                            "time/first_minibatch_sample"] = time.time(
                            ) - t_sample_start

                    if minibatch_count == num_minibatches:
                        time_metrics["time/last_minibatch_sample"] = time.time(
                        ) - t_sample_start

                    logger.info(
                        f"Training for batch {batch_idx}, minibatch {minibatch_count}/{num_minibatches}"
                    )
                    ...
                    logger.info(
                        f"Processed minibatch {minibatch_count}/{num_minibatches} with {len(minibatch_episodes)} episodes"
                    )

                optim_step_time = time.time()
                optim_step_future = await self.trainer.optim_step_future(
                    learning_rate=learning_rate,
                    beta1=beta1,
                    beta2=beta2,
                    eps=eps)
                ...
                time_metrics["time/forward_backward"] = sum(
                    forward_backward_times)
                time_metrics["time/one_batch_generate_and_train"] = time.time(
                ) - train_step_start

                # Print episodes
                print_episodes(episodes,
                               self.tokenizer,
                               num_episodes_to_print=2)
                ...
                metrics = compute_training_metrics(
                    ...
                    epoch=epoch,
                    training_datums=
                    training_datums,  # Pass datums for KL/perplexity metrics
                ...
                if self.val_dataloader and self.config.trainer.test_freq > 0 and batch_idx % self.config.trainer.test_freq == 0 and batch_idx > 0:
                    ...
                    val_metrics = await self.validate_agent(
                        self.val_dataloader, sampling_client)
                    if val_metrics:
                ...
                path_dict = await self.trainer.save_checkpoint_async(
                    batch_idx, kind="sampler")
                sampling_client = self.trainer.create_sampling_client(
                    path_dict["sampler_path"])
                time_metrics["time/save_sampler"] = time.time(
                ) - t_save_sampler_start

                # Checkpoint (full state) - skip if this is the resume batch
                if batch_idx % self.config.trainer.save_freq == 0:
                    logger.info(
                        f"Saving state checkpoint at batch {batch_idx}")
                    await self.trainer.save_checkpoint_async(batch_idx,
                                                             kind="state")
    ...
    def init_envs_and_agents(self, batch):
        ...
        full_agent_args = dict(self.config.agent.get("agent_args",
                                                     {})) | self.agent_args
        base_env_args = dict(self.config.env.get("env_args",
                                                 {})) | self.env_args

        def _create_env(i):
            if isinstance(env_args[i], str):
                env_args[i] = json.loads(env_args[i])
            return i, self.env_class.from_dict({
                **env_args[i],
                **base_env_args
            })
        ...
        with ThreadPoolExecutor(max_workers=64) as executor:
            env_futures = [
                executor.submit(_create_env, i) for i in range(len(env_args))
            ]
        ...
        with ThreadPoolExecutor(max_workers=64) as executor:
            agent_futures = [
                executor.submit(_create_agent, i) for i in range(len(envs))
            ]
    ...
    async def validate_agent(self, dataloader, sampling_client):
        ...
        mean_reward = sum([traj.reward for traj in all_trajectories
                           ]) / len(all_trajectories)
        std_reward = sum([
            (traj.reward - mean_reward)**2 for traj in all_trajectories
        ]) / len(all_trajectories)
        min_reward = min([traj.reward for traj in all_trajectories])
        max_reward = max([traj.reward for traj in all_trajectories])
        mean_turns = sum([len(traj.steps) for traj in all_trajectories
                          ]) / len(all_trajectories)
        return {
    ...
    async def generate_agent_episodes(self,
                                      timing_raw=None,
                                      meta_info=None,
                                      group_size: int = None,
        ...
        async def produce_episodes():
            ...
            async for traj in self.agent_execution_engine.trajectory_generator(
                ...
                if len(group_dict[traj["idx"] // group_size]) == group_size:
                    episode = self.convert_to_episode(group_dict[traj["idx"] //
                                                                 group_size])
                    await episode_queue.put(episode
                                            )  # Use await for async queue
        ...
        try:
            ...
            while True:
                # Collect minibatch
                try:
                    episode = await asyncio.wait_for(episode_queue.get(),
                                                     timeout=0.1)
